...
import globalAxios, { AxiosInstance, AxiosPromise } from "axios"; // Some imports not used depending on template conditions
// @ts-ignore
import {
	assertParamExists,
...
export const TaskApiAxiosParamCreator = function (
	...
	return {
		...
		taskControllerCopyTask: async (
			...
			const localVarPath = `/tasks/{id}/copy`.replace(
				`{${"id"}}`,
				encodeURIComponent(String(id))
			);
			// use dummy base URL string because the URL constructor only accepts absolute URLs.
...
export const TaskApiFactory = function (
	...
	return {
		...
		taskControllerCopyTask(
			...
			return localVarFp
				.taskControllerCopyTask(id, taskCopyApiParams, options)
				.then((request) => request(axios, basePath));
		},
		/**
		 *
