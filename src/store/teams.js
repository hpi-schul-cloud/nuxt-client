...
const module = mergeDeep(base, {
	actions: {
		acceptInvitation: async function (ctx, teamId) {
			return this.$axios.$get("/v1/teams/extern/accept/" + teamId);
		},
		getMyInvitations: async function () {
			return this.$axios.$get("/v1/teams/extern/get/");
		},
		inviteExternal: async function (ctx, payload) {
			return this.$axios.$patch("/v1/teams/extern/add/" + payload.teamId, {
		...
		deleteInvitation: async function (ctx, payload) {
			return this.$axios.$patch("/v1/teams/extern/remove/" + payload.teamId, {
				email: payload.email,
			});
		},
	},
