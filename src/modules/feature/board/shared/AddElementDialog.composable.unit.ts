...
import { createTestEnvStore, mockedPiniaStoreTyping, ObjectIdMock } from "@@/tests/test-utils";
	...
	describe("onElementClick", () => {
		describe("when element is created successfully", () => {
			const setup = () => {
				...
				const mockedBoardNotifierCalls = createMock<ReturnType<typeof useBoardNotifier>>();
			...
			describe("when element type is CollaborativeTextEditor", () => {
				...
				it("should show Notification", async () => {
					...
					const { addElementMock, elementType, mockedBoardNotifierCalls, cardId } = setup();
			...
			describe("when element type is NOT CollaborativeTextEditor or Whiteboard", () => {
				...
				it("should NOT show Notification", async () => {
					...
					expect(mockedBoardNotifierCalls.showCustomNotifier).toHaveBeenCalledTimes(0);
				});
			});

			describe("when element type is Whiteboard", () => {
				it("should show Notification", async () => {
					const i18nKeyWhiteboard = "components.cardElement.notification.visibleAndEditable";
					const addElementMock = vi.fn();
					...
					expect(mockedBoardNotifierCalls.showCustomNotifier).toHaveBeenCalledWith(i18nKeyWhiteboard, "info");
				});
			});
		});
