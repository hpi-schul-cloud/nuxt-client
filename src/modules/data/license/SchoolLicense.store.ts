...
import { MediaSchoolLicenseListResponse, MediaSchoolLicenseResponse } from "@/generated/serverApi/v3";
...
export const useSchoolLicenseStore = defineStore("schoolLicenseStore", () => {
	...
	const getLicenseIdentifier = (mediumId?: string, mediaSourceId?: string): string => `${mediumId} ${mediaSourceId}`;

	const fetchMediaSchoolLicenses = async (): Promise<void> => {
		isLoading.value = true;

		const licenses: MediaSchoolLicenseListResponse = await schoolLicenseApi.getMediaSchoolLicensesForSchool();

		const licenseIdentifiers: string[] = licenses.data.map((license: MediaSchoolLicenseResponse): string =>
	...
	const isLicensed = (mediumId?: string, mediaSourceId?: string): boolean => {
		const hasLicense: boolean = mediaSchoolLicenses.value.has(getLicenseIdentifier(mediumId, mediaSourceId));
