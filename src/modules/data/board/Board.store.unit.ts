...
const mockedUseBoardFocusHandler = vi.mocked(useBoardFocusHandler);
...
describe("BoardStore", () => {
	...
	const focusSetup = (id: string) => {
		...
		mockedUseBoardFocusHandler.mockReturnValue({
	...
	describe("createCardSuccess", () => {
		...
		it("should call setEditModeId", () => {
			...
			expect(mockedSharedEditMode.setEditModeId).toHaveBeenCalled();
		});
	});
	...
	describe("createColumnSuccess", () => {
		...
		it("should set focus if user initiated create column", () => {
			...
			expect(mockedSharedEditMode.setEditModeId).toHaveBeenCalledWith(NEW_COLUMN.id);
		});
	});
		...
		describe("when previous card needs to be focused", () => {
			...
			describe("when the card is first element", () => {
				it('should call "forceFocus" if already focused card is deleted', () => {
					...
					expect(mockedBoardFocusApi.forceFocus).toHaveBeenCalledWith(firstColumn.id);
				});
			});
			...
			describe("when the card is not the first element", () => {
				it('should call "forceFocus" if already focused card is deleted', () => {
					...
					expect(mockedBoardFocusApi.forceFocus).toHaveBeenCalledWith(firstCardId);
				});
			});
		});
	});
		...
		describe("when previous column needs to be focused", () => {
			...
			describe("when the column is the first element", () => {
				it('should call "forceFocus" if already focused column is deleted', () => {
					...
					expect(mockedBoardFocusApi.forceFocus).toHaveBeenCalledWith(boardStore.board?.id);
				});
			});
	...
	describe("moveCardToNewColumn", () => {
		...
		it("should call moveCardRequest from socket api if feature flag is disabled", async () => {
			...
			expect(mockedBoardSocketApi.moveCardRequest).toHaveBeenCalledWith({
				cardId,
	...
	describe("@FEATURE_COLUMN_BOARD_SOCKET_ENABLED", () => {
		...
		describe("@reloadBoard", () => {
			...
			it("should call socketApi.fetchBoardRequest when feature flag is set true", async () => {
				...
				expect(mockedBoardSocketApi.fetchBoardRequest).toHaveBeenCalledWith({
			...
			it("should call restApi.fetchBoardRequest when feature flag is set true", async () => {
				...
				expect(mockedBoardSocketApi.fetchBoardRequest).toHaveBeenCalledWith({ boardId: boardStore.board?.id });
			});
		});
		...
		describe("@deleteBoardRequest", () => {
			it("should call socketApi.deleteBoardRequest when feature flag is set true", async () => {
				...
				expect(mockedBoardSocketApi.deleteBoardRequest).toHaveBeenCalledWith({ boardId: "boardId" });
			});

			it("should call socketApi.deleteBoardRequest when feature flag is set false", async () => {
				const { boardStore } = setup();
