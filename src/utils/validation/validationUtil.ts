...
export const REGEX_H5P_ID = "[a-fA-F0-9]{24}|new";
...
export const isH5pId = (val: unknown): boolean =>
	typeof val === "string" && new RegExp(`^${REGEX_H5P_ID}$`).test(val);
...
export const oneOf =
	(...validators: ((val: unknown) => boolean)[]) =>
	(val: unknown): boolean =>
		validators.some((validator): boolean => validator(val));
