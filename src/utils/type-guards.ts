type EnsureKeysAreSet<T, K extends keyof T> = T & {
...
export class TypeGuard {
	...
	public static isStringOfStrings<T>(value: unknown, values: T[]): value is T {
		const isStringOfValue =
			TypeGuard.isString(value) && values.includes(value as T);
	...
	public static getValueFromObjectKey(value: unknown, key: string): unknown {
		...
		const result: unknown = TypeGuard.isEachKeyInObject(value, [key])
			? value[key]
			: undefined;
	...
	public static getValueFromDeepObjectKey(
		value: unknown,
		keyPath: string[]
	): unknown {
	...
	public static isEachKeyInObject<T extends Record<string, unknown>>(
		value: unknown,
		keys: (keyof T)[]
	): value is T {
	...
	public static checkKeyInObject<T>(
		value: T,
		key: string,
		toThrow?: Error
	): unknown {
	...
	public static checkKeysInInstance<T extends object, K extends keyof T>(
		...
		if (missingKeys.length > 0) {
			throw (
				toThrow || new Error(`Object lacks these properties: ${String(keys)}.`)
			);
		}
	...
	public static checkNotNullOrUndefined<T>(
