...
describe("ContextExternalToolSelectionRow", () => {
	let wrapper: Wrapper<any>;

	const setup = (maxHeight?: number, maxWidth?: number) => {
		document.body.setAttribute("data-app", "true");
		wrapper = shallowMount(ContextExternalToolSelectionRow, {
	...
	describe("when component is used", () => {
		it("should be found in the dom", () => {
			setup();
			expect(
				wrapper.findComponent(ContextExternalToolSelectionRow).exists()
			).toBeTruthy();
		});
	});
