...
const isMigrationNotStarted = computed(
  () => school.value.inUserMigration === undefined,
);

const canPerformMigration = computed(
  () => school.value.inUserMigration && school.value.inMaintenance,
);

const isMigrationFinished = computed(
  () => school.value.inUserMigration === false,
);

const canFinishMaintenance = computed(
  () => isMigrationConfirm.value || isMigrationFinished.value,
);
...
const isBrb = computed(
  () => useEnvConfig().value.SC_THEME.toLowerCase() === SchulcloudTheme.Brb,
);
