...
export default {
	...
	computed: {
		...
		filteredColumns() {
			...
			if (this.schoolIsExternallyManaged || !authModule.getUserRoles.some((name) => name === "administrator")) {
		...
		fab() {
			if (this.schoolIsExternallyManaged || !this.$_userHasPermission("TEACHER_CREATE")) {
				return null;
			}
