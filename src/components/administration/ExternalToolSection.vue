...
import { AUTH_MODULE_KEY, injectStrict, NOTIFIER_MODULE_KEY, SCHOOL_EXTERNAL_TOOLS_MODULE_KEY } from "@/utils/inject";
...
const items: ComputedRef<SchoolExternalToolItem[]> = computed(() => getItems(schoolExternalToolsModule));

const isLoading: ComputedRef<boolean> = computed(() => schoolExternalToolsModule.getLoading);
...
const onDeleteTool = async () => {
	...
	notifierModule.show({
		text: t("components.administration.externalToolsSection.notification.deleted"),
		status: "success",
	});
...
const getItemName: ComputedRef<string> = computed(() => (itemToDelete.value ? itemToDelete.value?.name : ""));
...
const openDeleteDialog = async (item: SchoolExternalToolItem) => {
	...
	if (!metadata.value) {
		notifierModule.show({
			text: t("components.administration.externalToolsSection.dialog.content.metadata.error"),
			status: "error",
		});
	}
};
...
const isLicensedToSchool = (mediumId?: string, mediaSourceId?: string): boolean =>
	schoolLicenseStore.isLicensed(mediumId, mediaSourceId);

const isLicensesLoading: ComputedRef<boolean> = computed(() => schoolLicenseStore.isLoading);
</script>
