...
describe("@components/organisms/DataTable/BackendDataTable", () => {
	...
	describe("rendering", () => {
		it("Passing the columns and data should render the full table on non-mobile devices. Nested properties should be possible.", () => {
			...
			if (window.length > mobile) {
				...
				expect(wrapper.find("thead tr div").text()).toContain("Vorname");
				expect(
					wrapper.find("tbody tr").findAll(".text-content").at(2).text()
				).toContain(defaultData[0].address.city);
			} else {
				expect(wrapper.find("thead tr").length).toBeUndefined();
			}

			expect(wrapper.find("tbody tr").contains(".text-content")).toBe(true);

			expect(wrapper.find("tbody tr div").text()).toContain(
	...
	if (window.length > mobile) {
		describe("sort", () => {
			...
			it("should emit sort events on click on sortable column", async () => {
				const wrapper = getWrapper({
					data: defaultData,
				});
		...
		describe("selection", () => {
			...
			it("can preselect all values", async () => {
				const totalSelections = testData.length;
				const expectedSelection = [...Array(totalSelections).keys()].map(
					String
				);
				const wrapper = getWrapper({
					data: testData,
			...
			describe("header checkbox shows", () => {
				...
				describe("on exclusive selection", () => {
					const getWrapperLocal = (numberOfUnselections) => {
						const selection = [...Array(numberOfUnselections).keys()].map(
							String
						);
						return getWrapper(
							{
