  ...
  export default {
    ...
    methods: {
      ...
      setupMessenger(session) {
        ...
        const { roomType, roomId } = this.extractRoomTypeAndIdFromPath(window.location.pathname);
        const servername = this.extractServernameFromMatrixUserId(matrixUserId);
        const matrixRoomId = this.composeMatrixRoomId(roomType, roomId, servername);
