  ...
  <div>
    <v-card v-ripple="false">
      <v-toolbar dark color="primary">
        <v-toolbar-title>
          Verknüpfe <strong>weBBschule-Konto</strong> mit {{ this.$theme.short_name }} Konto
        </v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
      ...
      <v-card-text class="mt-5">
        Das webbschule-Konto wird später in die {{ this.$theme.short_name }} importiert.
        Wenn Sie das Konto mit einem bestehenden {{ this.$theme.short_name }} Konto verknüpfen möchten, so dass die
        Benutzerdaten erhalten bleiben, wählen Sie hier das {{ this.$theme.short_name }} Konto aus. Andernfalls wird
        dieses als neues Konto angeleg.
      </v-card-text>
      <v-card-text>
        <v-row>
          <v-col class="md-6">
            ...
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title>{{ editedItem.firstName }} {{ editedItem.lastName }}</v-list-item-title>
                <v-list-item-subtitle>Rolle: {{ editedItem.roleNames.join(', ') }}</v-list-item-subtitle>
                <v-list-item-subtitle>Klasse: {{ editedItem.classNames.join(', ') }}</v-list-item-subtitle>
                <v-list-item-subtitle>Nutzername: {{ editedItem.loginName }}</v-list-item-subtitle>
              </v-list-item-content>
              <v-list-item-content>
              </v-list-item-content>
            </v-list-item>
          ...
          <v-col class="md-6">
            ...
            <v-list-item-content v-else-if="editedItem.match">
              ...
              <v-list-item-subtitle>
                {{ editedItem.match.roleNames ? editedItem.match.roleNames.join(", ") : "" }}
              </v-list-item-subtitle>
              <v-list-item-subtitle>
      ...
      <v-card-actions>
        ...
        <v-col class="col-6 text-right">
          ...
          <v-btn text :class="canDelete ? 'secondary' : ''" class="m-2" :disabled="!canDelete"
                 @click="deleteMatch">
...
export default {
  ...
  computed: {
    ...
    canSave() {
      ...
      if (this.editedItem.match && this.editedItem.match.userId === this.selectedItem.userId) {
        return false;
      }
      return true;
    },
  ...
  methods: {
    ...
    async flag() {
      await ImportUserModule.saveFlag({ importUserId: this.editedItem.importUserId, flagged: !this.editedItem.flagged });
    },
  },
};
</script>
